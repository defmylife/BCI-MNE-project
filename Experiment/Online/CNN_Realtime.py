...
from module.gripper import Gripper
...
def main():
    ...
    CNN = FFT_CNNModel(input_channels=3)
    ...
    while True:
        ...
        if timestamps:
            ...
            data = [eeg_sample[1],eeg_sample[2],eeg_sample[3]]
            temp.append(data)
            ...
            if time_count == 1.0:
                ...
                send_epoch = epoch_5s
                predict_raw_value, predict_value = CNN.model_predict_classes(np.reshape(send_epoch, (1, 3, 1250)))
                print('-----------------------------------------------------------------------------------')
