...
#Global Variab
L_blink = Display_Hz/5
R_blink = Display_Hz/7
num_repeat_trails = 8
squareSize = 0.25
...
Repeat_32_Trials = data.TrialHandler(nReps= num_repeat_trails, method='random', 
    ...
    trialList=data.importConditions('position_of_red_rectangle25.xlsx'),
...
for thisRepeat_32_Trial in Repeat_32_Trials:
    ...
    while continueRoutine and routineTimer.getTime() < end_time_Cue:
        ...
        ####### Tell psychopy to send event marker ##########
        if frameN ==1:
            if position_red_x == -0.25 and position_red_y == 0:
                outlet.push_sample([1]) # left cue = marker no.1
            elif position_red_x == 0.25 and position_red_y == 0:
